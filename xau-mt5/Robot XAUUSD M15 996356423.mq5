...
static input int    Magic_Number = 996356423; // Magic number
static input double Entry_Amount =     0.01; // Entry lots
       input int    Stop_Loss    =     3492; // Stop Loss   (pips)
       input int    Take_Profit  =     4166; // Take Profit (pips)

static input string ___0______   = "------"; // --- Candle Color ---
       input int    Ind0Param0   =        6; // Min body height [pips]
       input int    Ind0Param1   =        4; // Consecutive candles

static input string ___1______   = "------"; // --- Standard Deviation ---
       input int    Ind1Param0   =        6; // Period
       input double Ind1Param1   =  15.0000; // Level

static input string ___2______   = "------"; // --- DeMarker ---
       input int    Ind2Param0   =        7; // Period
       input double Ind2Param1   =     0.64; // Level

static input string ___3______   = "------"; // --- Alligator ---
       input int    Ind3Param0   =       35; // Jaws period
       input int    Ind3Param1   =       14; // Jaws shift
       input int    Ind3Param2   =       14; // Teeth period
       input int    Ind3Param3   =       11; // Teeth shift
       input int    Ind3Param4   =       11; // Lips period
       input int    Ind3Param5   =        2; // Lips shift
...
void InitIndicators(void)
  {
   ...
   // Standard Deviation (Close, Simple, 6), Level: 15.0000
   indHandlers[0][1][0] = iStdDev(NULL, 0, Ind1Param0, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][1][0]);
   // DeMarker (7), Level: 0.64
   indHandlers[0][2][0] = iDeMarker(NULL, 0, Ind2Param0);
   if(Show_inds) ChartIndicatorAdd(0, 2, indHandlers[0][2][0]);
   // Alligator (Smoothed, Median, 35, 14, 14, 11, 11, 2)
   indHandlers[0][3][0] = iAlligator(NULL, 0, Ind3Param0, Ind3Param1, Ind3Param2, Ind3Param3, Ind3Param4, Ind3Param5, MODE_SMMA, PRICE_MEDIAN);
...
void ManageClose(void)
  {
   ...
   double ind3buffer2[]; CopyBuffer(indHandlers[0][3][0], 2, 1, 2, ind3buffer2);
   double ind3val1  = ind3buffer1[1];
   double ind3val2  = ind3buffer0[1];
   double ind3val3  = ind3buffer1[0];
...
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":3492,"takeProfit":4166,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Candle Color","listIndexes":[0,0,0,0,0],"numValues":[6,4,0,0,0,0]},{"name":"Standard Deviation","listIndexes":[3,3,0,0,0],"numValues":[6,15,0,0,0,0]},{"name":"DeMarker","listIndexes":[2,0,0,0,0],"numValues":[7,0.64,0,0,0,0]}],"closeFilters":[{"name":"Alligator","listIndexes":[11,3,4,0,0],"numValues":[35,14,14,11,11,2]}]} */
