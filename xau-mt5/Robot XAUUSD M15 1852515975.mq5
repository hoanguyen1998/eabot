...
static input int    Magic_Number = 1852515975; // Magic number
static input double Entry_Amount =     0.01; // Entry lots
       input int    Stop_Loss    =     4195; // Stop Loss   (pips)
       input int    Take_Profit  =     4346; // Take Profit (pips)

static input string ___0______   = "------"; // --- Moving Averages Crossover ---
       input int    Ind0Param0   =        7; // Fast MA period
       input int    Ind0Param1   =       19; // Slow MA period

static input string ___1______   = "------"; // --- Awesome Oscillator ---
       input double Ind1Param0   =   0.0000; // Level

static input string ___2______   = "------"; // --- Directional Indicators ---
       input int    Ind2Param0   =       17; // Period

static input string ___3______   = "------"; // --- Commodity Channel Index ---
       input int    Ind3Param0   =       12; // Period
       input int    Ind3Param1   =      100; // Level

static input string ___4______   = "------"; // --- Alligator ---
       input int    Ind4Param0   =       29; // Jaws period
       input int    Ind4Param1   =        8; // Jaws shift
       input int    Ind4Param2   =        8; // Teeth period
       input int    Ind4Param3   =        4; // Teeth shift
       input int    Ind4Param4   =        4; // Lips period
       input int    Ind4Param5   =        1; // Lips shift
...
void InitIndicators(void)
  {
   // Moving Averages Crossover (Simple, Simple, 7, 19)
   indHandlers[0][0][0] = iMA(NULL, 0, Ind0Param0, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][0][0]);
   // Moving Averages Crossover (Simple, Simple, 7, 19)
   indHandlers[0][0][1] = iMA(NULL, 0, Ind0Param1, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][0][1]);
   ...
   indHandlers[0][3][0] = iCCI(NULL, 0, Ind3Param0, PRICE_TYPICAL);
   if(Show_inds) ChartIndicatorAdd(0, 3, indHandlers[0][3][0]);
   // Alligator (Smoothed, Median, 29, 8, 8, 4, 4, 1)
...
int GetEntrySignal(void)
  {
   ...
   double ind0buffer1[]; CopyBuffer(indHandlers[0][0][1], 0, 1, 2, ind0buffer1);
...
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":4195,"takeProfit":4346,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":true},"openFilters":[{"name":"Moving Averages Crossover","listIndexes":[1,0,0,0,0],"numValues":[7,19,0,0,0,0]},{"name":"Awesome Oscillator","listIndexes":[2,0,0,0,0],"numValues":[0,0,0,0,0,0]},{"name":"Directional Indicators","listIndexes":[2,0,0,0,0],"numValues":[17,0,0,0,0,0]},{"name":"Commodity Channel Index","listIndexes":[0,5,0,0,0],"numValues":[12,100,0,0,0,0]}],"closeFilters":[{"name":"Alligator","listIndexes":[10,3,4,0,0],"numValues":[29,8,8,4,4,1]}]} */
