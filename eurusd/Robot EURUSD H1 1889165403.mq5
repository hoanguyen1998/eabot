...
static input int    Magic_Number = 1889165403; // Magic number
...
static input string ___0______   = "------"; // --- Momentum ---
       input int    Ind0Param0   =       24; // Period
       input double Ind0Param1   = 100.0000; // Level

static input string ___1______   = "------"; // --- Stochastic ---
       input int    Ind1Param0   =       15; // %K Period
       input int    Ind1Param1   =       10; // %D Period
       input int    Ind1Param2   =        7; // Slowing
       input double Ind1Param3   =     50.0; // Level

static input string ___2______   = "------"; // --- ADX ---
       input int    Ind2Param0   =       37; // Period
       input double Ind2Param1   =      0.0; // Level

static input string ___3______   = "------"; // --- Moving Average of Oscillator ---
       input int    Ind3Param0   =       17; // Fast EMA
       input int    Ind3Param1   =       24; // Slow EMA
       input int    Ind3Param2   =        3; // MACD SMA
       input double Ind3Param3   =   0.0000; // Level
...
void InitIndicators(void)
  {
   // Momentum (Close, 24)
   indHandlers[0][0][0] = iMomentum(NULL, 0, Ind0Param0, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][0][0]);
   // Stochastic (15, 10, 7)
   indHandlers[0][1][0] = iStochastic(NULL, 0, Ind1Param0, Ind1Param1, Ind1Param2, MODE_SMA, 0);
   ...
   // Moving Average of Oscillator (Close, 17, 24, 3), Level: 0.0000
   indHandlers[0][3][0] = iOsMA(NULL, 0, Ind3Param0, Ind3Param1, Ind3Param2, PRICE_CLOSE);
...
int GetEntrySignal(void)
  {
   ...
   // Stochastic (15, 10, 7)
   double ind1buffer[]; CopyBuffer(indHandlers[0][1][0], MAIN_LINE, 1, 3, ind1buffer);
...
void ManageClose(void)
  {
   ...
   bool   ind3short = ind3val1 > -Ind3Param3 + sigma;
...
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":65,"takeProfit":66,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Momentum","listIndexes":[7,3,0,0,0],"numValues":[24,100,0,0,0,0]},{"name":"Stochastic","listIndexes":[1,0,0,0,0],"numValues":[15,10,7,50,0,0]},{"name":"ADX","listIndexes":[1,0,0,0,0],"numValues":[37,0,0,0,0,0]}],"closeFilters":[{"name":"Moving Average of Oscillator","listIndexes":[3,3,0,0,0],"numValues":[17,24,3,-0.0011,0,0]}]} */
