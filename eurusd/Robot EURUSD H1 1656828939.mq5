...
static input int    Magic_Number = 1656828939; // Magic number
static input double Entry_Amount =     0.01; // Entry lots
       input int    Stop_Loss    =       95; // Stop Loss   (pips)
       input int    Take_Profit  =       97; // Take Profit (pips)

static input string ___0______   = "------"; // --- Moving Averages Crossover ---
       input int    Ind0Param0   =        6; // Fast MA period
       input int    Ind0Param1   =       13; // Slow MA period

static input string ___1______   = "------"; // --- Average True Range ---
       input int    Ind1Param0   =        2; // Period
       input double Ind1Param1   =   0.0010; // Level

static input string ___2______   = "------"; // --- Moving Average of Oscillator ---
       input int    Ind2Param0   =       14; // Fast EMA
       input int    Ind2Param1   =       21; // Slow EMA
       input int    Ind2Param2   =        9; // MACD SMA
       input double Ind2Param3   =   0.0000; // Level

static input string ___3______   = "------"; // --- Bulls Power ---
       input int    Ind3Param0   =       18; // Period
       input double Ind3Param1   =  -0.0167; // Level
...
void InitIndicators(void)
  {
   // Moving Averages Crossover (Simple, Simple, 6, 13)
   ...
   // Average True Range (2), Level: 0.0010
   indHandlers[0][1][0] = iATR(NULL, 0, Ind1Param0);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][1][0]);
   // Moving Average of Oscillator (Close, 14, 21, 9), Level: 0.0000
   indHandlers[0][2][0] = iOsMA(NULL, 0, Ind2Param0, Ind2Param1, Ind2Param2, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 2, indHandlers[0][2][0]);
   // Bulls Power (18), Level: -0.0167
...
int GetEntrySignal(void)
  {
   ...
   bool   ind2short = ind2val1 > -Ind2Param3 + sigma;
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET BlackBull; EURUSD; H1 */
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":95,"takeProfit":97,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Moving Averages Crossover","listIndexes":[1,0,0,0,0],"numValues":[6,13,0,0,0,0]},{"name":"Average True Range","listIndexes":[2,0,0,0,0],"numValues":[2,0.001,0,0,0,0]},{"name":"Moving Average of Oscillator","listIndexes":[3,3,0,0,0],"numValues":[14,21,9,0,0,0]}],"closeFilters":[{"name":"Bulls Power","listIndexes":[3,0,0,0,0],"numValues":[18,-0.0167,0,0,0,0]}]} */
