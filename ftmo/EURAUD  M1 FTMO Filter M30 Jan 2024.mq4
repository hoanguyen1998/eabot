...
input int Stop_Loss   = 300; // Stop Loss [point]
input int Take_Profit = 600; // Take Profit [point]
input int Break_Even  = 450; // Break Even [point]
static input double Martingale_Multiplier = 1; // Martingale Multiplier

static input string IndicatorName0 = "Envelopes"; // ------ Indicator parameters ------
input int Slot0IndParam0 = 4; // MA period
input double Slot0IndParam1 = 0.07; // Deviation %
static input string IndicatorName1 = "MACD"; // ------ Indicator parameters ------
input int Slot1IndParam0 = 13; // Slow MA period
input int Slot1IndParam1 = 6; // Fast MA period
input int Slot1IndParam2 = 3; // Signal line period
static input string IndicatorName2 = "Spread Level Pro"; // ------ Indicator parameters ------
input int Slot2IndParam0 = 30; // Spread level in points
static input string IndicatorName3 = "Chande Momentum Oscillator"; // ------ Indicator parameters ------
input int Slot3IndParam0 = 75; // Smoothing period
input int Slot3IndParam1 = 30; // Level
static input string IndicatorName4 = "Entry Time"; // ------ Indicator parameters ------
input int Slot4IndParam0 = 1; // From hour (incl.)
input int Slot4IndParam1 = 0; // From min (incl.)
input int Slot4IndParam2 = 23; // Until hour (excl.)
input int Slot4IndParam3 = 0; // Until min (excl.)
static input string IndicatorName5 = "Account Percent Stop"; // ------ Indicator parameters ------
input double Slot5IndParam0 = 4.9; // Account percent


static input string ExpertSettings__ = "------------"; // ------ Expert Settings ------

// A unique number of the expert's orders.
static input int Expert_Magic = 23143154; // Expert Magic Number
...
void Envelopes::Calculate(DataSet &dataSet)
  {
   ...
   if(SlotType==SlotTypes_Open || SlotType==SlotTypes_Close)
   ...
   else
     {
      ...
      else if(ListParam[0].Text=="The bar closes below Upper Band after closing above it")
         BandIndicatorLogic(firstBar,previous,upperBand,lowerBand,Component[3],Component[4],BandIndLogic_The_bar_closes_below_Upper_Band_after_above);
      else if(ListParam[0].Text=="The bar closes above Upper Band after closing below it")
         BandIndicatorLogic(firstBar,previous,upperBand,lowerBand,Component[3],Component[4],BandIndLogic_The_bar_closes_above_Upper_Band_after_below);
      else if(ListParam[0].Text=="The bar closes below Lower Band after closing above it")
         BandIndicatorLogic(firstBar,previous,upperBand,lowerBand,Component[3],Component[4],BandIndLogic_The_bar_closes_below_Lower_Band_after_above);
      else if(ListParam[0].Text=="The bar closes above Lower Band after closing below it")
         BandIndicatorLogic(firstBar,previous,upperBand,lowerBand,Component[3],Component[4],BandIndLogic_The_bar_closes_above_Lower_Band_after_below);
     }
  }
//+------------------------------------------------------------------+
...
class SpreadLevelPro : public Indicator
  {
public:
   SpreadLevelPro(SlotTypes slotType)
     {
      SlotType=slotType;

      IndicatorName="Spread Level Pro";
...
//|                                                                  |
//+------------------------------------------------------------------+
void SpreadLevelPro::Calculate(DataSet &dataSet)
  {
   Data=GetPointer(dataSet);

// Reading the parameters
   double spreadLevel=NumParam[0].Value;

// Calculation
   const int firstBar=1;
   double spread=(Data.Ask-Data.Bid)/Data.Point;

   double spr[]; ArrayResize(spr,Data.Bars);ArrayInitialize(spr,0);

   if(ListParam[0].Text=="Spread is lower than the selected level")
     {
      if(spread<spreadLevel-Epsilon())
        ArrayInitialize(spr,1);
     }
   else if(ListParam[0].Text=="Spread is higher than the selected level")
     {
      if(spread>spreadLevel+Epsilon())
        ArrayInitialize(spr,1);
     }
   ...
   ArrayResize(Component[0].Value,Data.Bars);
   Component[0].CompName = "Spread";
   Component[0].DataType = IndComponentType_IndicatorValue;
   Component[0].FirstBar = firstBar;
   ArrayInitialize(Component[0].Value,spread);
   ...
   if(SlotType==SlotTypes_OpenFilter)
     {
      ...
      Component[1].FirstBar = firstBar;
      ArrayCopy(Component[1].Value,spr);

      ArrayResize(Component[2].Value,Data.Bars);
      Component[2].CompName = "Is short entry allowed";
      Component[2].DataType = IndComponentType_AllowOpenShort;
      Component[2].FirstBar = firstBar;
      ArrayCopy(Component[2].Value,spr);
     }
   else if(SlotType==SlotTypes_CloseFilter)
     {
      ArrayResize(Component[1].Value,Data.Bars);
      Component[1].CompName = "Force close";
      Component[1].DataType = IndComponentType_ForceClose;
      Component[1].FirstBar = firstBar;
      ArrayCopy(Component[1].Value,spr);
     }
  }
...
Indicator *IndicatorManager::CreateIndicator(string indicatorName,SlotTypes slotType)
  {
   ...
   if (indicatorName == "Spread Level Pro") return new SpreadLevelPro(slotType);
if (indicatorName == "Chande Momentum Oscillator") return new ChandeMomentumOscillator(slotType);
...
Strategy *StrategyManager::GetStrategy(void)
  {
   ...
   strategy.StrategyName           = "EURAUD  M1 FTMO Filter M30 (3)";
   ...
   strategy.Description            = "Exported on 23.12.2024 г. from Forex Strategy Builder Professional, v4.3.3";
   strategy.RecommendedBars        = 1000;
   strategy.FirstBar               = 78;
   strategy.MinBarsRequired        = 81;
   ...
   strategy.Slot[2].IndicatorName    = "Spread Level Pro";
   ...
   strategy.Slot[2].LogicalGroup     = "All";
   strategy.Slot[2].IndicatorPointer = indicatorManager.CreateIndicator("Spread Level Pro", SlotTypes_OpenFilter);
   ...
   strategy.Slot[2].IndicatorPointer.ListParam[0].Text    = "Spread is lower than the selected level";

   strategy.Slot[2].IndicatorPointer.NumParam[0].Enabled = true;
   strategy.Slot[2].IndicatorPointer.NumParam[0].Caption = "Spread level in points";
   strategy.Slot[2].IndicatorPointer.NumParam[0].Value   = Slot2IndParam0;

   strategy.Slot[3].IndicatorName    = "Chande Momentum Oscillator";
   ...
   strategy.Slot[3].IndicatorPointer = indicatorManager.CreateIndicator("Chande Momentum Oscillator", SlotTypes_OpenFilter);

   strategy.Slot[3].IndicatorPointer.ListParam[0].Enabled = true;
   strategy.Slot[3].IndicatorPointer.ListParam[0].Caption = "Logic";
   strategy.Slot[3].IndicatorPointer.ListParam[0].Index   = 1;
   strategy.Slot[3].IndicatorPointer.ListParam[0].Text    = "CMO falls";
   ...
   strategy.Slot[4].IndicatorName    = "Entry Time";
   ...
   strategy.Slot[4].LogicalGroup     = "All";
   strategy.Slot[4].IndicatorPointer = indicatorManager.CreateIndicator("Entry Time", SlotTypes_OpenFilter);
   ...
   strategy.Slot[4].IndicatorPointer.ListParam[0].Text    = "Enter the market between the specified hours";

   strategy.Slot[4].IndicatorPointer.NumParam[0].Enabled = true;
   strategy.Slot[4].IndicatorPointer.NumParam[0].Caption = "From hour (incl.)";
   strategy.Slot[4].IndicatorPointer.NumParam[0].Value   = Slot4IndParam0;
   strategy.Slot[4].IndicatorPointer.NumParam[1].Enabled = true;
   strategy.Slot[4].IndicatorPointer.NumParam[1].Caption = "From min (incl.)";
   strategy.Slot[4].IndicatorPointer.NumParam[1].Value   = Slot4IndParam1;
   strategy.Slot[4].IndicatorPointer.NumParam[2].Enabled = true;
   strategy.Slot[4].IndicatorPointer.NumParam[2].Caption = "Until hour (excl.)";
   strategy.Slot[4].IndicatorPointer.NumParam[2].Value   = Slot4IndParam2;
   strategy.Slot[4].IndicatorPointer.NumParam[3].Enabled = true;
   strategy.Slot[4].IndicatorPointer.NumParam[3].Caption = "Until min (excl.)";
   strategy.Slot[4].IndicatorPointer.NumParam[3].Value   = Slot4IndParam3;

   strategy.Slot[5].IndicatorName    = "Account Percent Stop";
   ...
   strategy.Slot[5].IndicatorPointer = indicatorManager.CreateIndicator("Account Percent Stop", SlotTypes_Close);
   ...
   strategy.Slot[5].IndicatorPointer.ListParam[0].Text    = "Limit the risk to percent of the account";

   strategy.Slot[5].IndicatorPointer.NumParam[0].Enabled = true;
   strategy.Slot[5].IndicatorPointer.NumParam[0].Caption = "Account percent";
...
/*STRATEGY CODE <?xml version=\"1.0\"?><strategy><programName>Forex Strategy Builder Professional</programName><programVersion>v4.3.3</programVersion><strategyName>EURAUD  M1 FTMO Filter M30 (3)</strategyName><profileName>Default profile</profileName><dataSourceName>FSB Demo data</dataSourceName><instrumentSymbol>EURUSD</instrumentSymbol><instrumentPeriod>1</instrumentPeriod><sameDirSignalAction>Loser</sameDirSignalAction><oppDirSignalAction>Reverse</oppDirSignalAction><maxOpenLots>1</maxOpenLots><useAccountPercentEntry>False</useAccountPercentEntry><entryLots>1</entryLots><addingLots>1</addingLots><reducingLots>1</reducingLots><useMartingale>True</useMartingale><martingaleMultiplier>1</martingaleMultiplier><description>Exported on 23.12.2024 г. from Forex Strategy Builder Professional, v4.3.3</description><recommendedBars>1000</recommendedBars><openFilters>4</openFilters><closeFilters>0</closeFilters><firstBar>78</firstBar><minBarsRequired>81</minBarsRequired><permanentStopLoss usePermanentSL=\"True\" permanentSLType=\"Relative\">300</permanentStopLoss><permanentTakeProfit usePermanentTP=\"True\" permanentTPType=\"Relative\">600</permanentTakeProfit><breakEven useBreakEven=\"True\">450</breakEven><slot slotNumber=\"0\" slotType=\"Open\"><indicatorName>Envelopes</indicatorName><listParam paramNumber=\"0\"><caption>Logic</caption><index>1</index><value>Enter long at Lower Band</value></listParam><listParam paramNumber=\"1\"><caption>Smoothing method</caption><index>0</index><value>Simple</value></listParam><listParam paramNumber=\"2\"><caption>Base price</caption><index>3</index><value>Close</value></listParam><numParam paramNumber=\"0\"><caption>MA period</caption><value>4</value></numParam><numParam paramNumber=\"1\"><caption>Deviation %</caption><value>0.07</value></numParam><checkParam paramNumber=\"0\"><caption>Use previous bar value</caption><value>True</value></checkParam><signalShift>0</signalShift><signalRepeat>0</signalRepeat><indicatorSymbol></indicatorSymbol><indicatorPeriod>M1</indicatorPeriod></slot><slot slotNumber=\"1\" slotType=\"OpenFilter\" logicalGroup=\"A\"><indicatorName>MACD</indicatorName><listParam paramNumber=\"0\"><caption>Logic</caption><index>11</index><value>MACD line is lower than the Signal line</value></listParam><listParam paramNumber=\"1\"><caption>Smoothing method</caption><index>1</index><value>Exponential</value></listParam><listParam paramNumber=\"2\"><caption>Base price</caption><index>3</index><value>Close</value></listParam><listParam paramNumber=\"3\"><caption>Signal line method</caption><index>0</index><value>Simple</value></listParam><numParam paramNumber=\"0\"><caption>Slow MA period</caption><value>13</value></numParam><numParam paramNumber=\"1\"><caption>Fast MA period</caption><value>6</value></numParam><numParam paramNumber=\"2\"><caption>Signal line period</caption><value>3</value></numParam><checkParam paramNumber=\"0\"><caption>Use previous bar value</caption><value>True</value></checkParam><signalShift>0</signalShift><signalRepeat>0</signalRepeat><indicatorSymbol></indicatorSymbol><indicatorPeriod>M30</indicatorPeriod></slot><slot slotNumber=\"2\" slotType=\"OpenFilter\" logicalGroup=\"All\"><indicatorName>Spread Level Pro</indicatorName><listParam paramNumber=\"0\"><caption>Logic</caption><index>0</index><value>Spread is lower than the selected level</value></listParam><numParam paramNumber=\"0\"><caption>Spread level in points</caption><value>30</value></numParam><signalShift>0</signalShift><signalRepeat>0</signalRepeat><indicatorSymbol></indicatorSymbol><indicatorPeriod>M1</indicatorPeriod></slot><slot slotNumber=\"3\" slotType=\"OpenFilter\" logicalGroup=\"A\"><indicatorName>Chande Momentum Oscillator</indicatorName><listParam paramNumber=\"0\"><caption>Logic</caption><index>1</index><value>CMO falls</value></listParam><listParam paramNumber=\"2\"><caption>Base price</caption><index>3</index><value>Close</value></listParam><numParam paramNumber=\"0\"><caption>Smoothing period</caption><value>75</value></numParam><numParam paramNumber=\"1\"><caption>Level</caption><value>30</value></numParam><checkParam paramNumber=\"0\"><caption>Use previous bar value</caption><value>True</value></checkParam><signalShift>0</signalShift><signalRepeat>0</signalRepeat><indicatorSymbol></indicatorSymbol><indicatorPeriod>M1</indicatorPeriod></slot><slot slotNumber=\"4\" slotType=\"OpenFilter\" logicalGroup=\"All\"><indicatorName>Entry Time</indicatorName><listParam paramNumber=\"0\"><caption>Logic</caption><index>0</index><value>Enter the market between the specified hours</value></listParam><numParam paramNumber=\"0\"><caption>From hour (incl.)</caption><value>1</value></numParam><numParam paramNumber=\"1\"><caption>From min (incl.)</caption><value>0</value></numParam><numParam paramNumber=\"2\"><caption>Until hour (excl.)</caption><value>23</value></numParam><numParam paramNumber=\"3\"><caption>Until min (excl.)</caption><value>0</value></numParam></slot><slot slotNumber=\"5\" slotType=\"Close\"><indicatorName>Account Percent Stop</indicatorName><listParam paramNumber=\"0\"><caption>Logic</caption><index>0</index><value>Limit the risk to percent of the account</value></listParam><numParam paramNumber=\"0\"><caption>Account percent</caption><value>4.90</value></numParam></slot></strategy> */
//+------------------------------------------------------------------+
