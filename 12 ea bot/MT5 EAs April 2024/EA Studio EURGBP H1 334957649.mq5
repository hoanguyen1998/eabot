...
static input int    Magic_Number = 334957649; // Magic number
static input double Entry_Amount =     0.10; // Entry lots
       input int    Stop_Loss    =       67; // Stop Loss   (pips)
       input int    Take_Profit  =       31; // Take Profit (pips)

static input string ___0______   = "------"; // --- Alligator ---
       input int    Ind0Param0   =       38; // Jaws period
       input int    Ind0Param1   =       23; // Jaws shift
       input int    Ind0Param2   =       23; // Teeth period
       input int    Ind0Param3   =       10; // Teeth shift
       input int    Ind0Param4   =       10; // Lips period
       input int    Ind0Param5   =        2; // Lips shift

static input string ___1______   = "------"; // --- RVI Signal ---
       input int    Ind1Param0   =       42; // Period

static input string ___2______   = "------"; // --- Force Index ---
       input int    Ind2Param0   =       38; // Period

static input string ___3______   = "------"; // --- Moving Average ---
       input int    Ind3Param0   =       36; // Period
       input int    Ind3Param1   =        0; // Shift

static input string ___4______   = "------"; // --- Standard Deviation ---
       input int    Ind4Param0   =       28; // Period
       input double Ind4Param1   =   0.0141; // Level
...
void InitIndicators(void)
  {
   // Alligator (Smoothed, Median, 38, 23, 23, 10, 10, 2)
   ...
   // RVI Signal (42)
   indHandlers[0][1][0] = iRVI(NULL, 0, Ind1Param0);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][1][0]);
   // Force Index (Simple, 38)
   indHandlers[0][2][0] = iForce(NULL, 0, Ind2Param0, MODE_SMA, VOLUME_TICK);
   if(Show_inds) ChartIndicatorAdd(0, 2, indHandlers[0][2][0]);
   // Moving Average (Simple, Close, 36, 0)
   indHandlers[0][3][0] = iMA(NULL, 0, Ind3Param0, Ind3Param1, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][3][0]);
   // Standard Deviation (Close, Simple, 28), Level: 0.0141
   indHandlers[0][4][0] = iStdDev(NULL, 0, Ind4Param0, 0, MODE_SMA, PRICE_CLOSE);
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET Premium Data; EURGBP; H1 */
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":67,"takeProfit":31,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Alligator","listIndexes":[10,3,4,0,0],"numValues":[38,23,23,10,10,2]},{"name":"RVI Signal","listIndexes":[3,0,0,0,0],"numValues":[42,0,0,0,0,0]},{"name":"Force Index","listIndexes":[2,0,0,0,0],"numValues":[38,0,0,0,0,0]}],"closeFilters":[{"name":"Moving Average","listIndexes":[4,0,3,0,0],"numValues":[36,0,0,0,0,0]},{"name":"Standard Deviation","listIndexes":[2,3,0,0,0],"numValues":[28,0.0141,0,0,0,0]}]} */
